...
// Two things over UPnP, both on the home network only:
//
//   routerTable()  reads the router's forwarded-port table, the core of the
//                  exposure check: a device is visible from the internet when
//                  one of its ports is in this table.
//   discoverAll()  asks every device that speaks UPnP to announce itself, which
//                  finds TVs, printers and streaming boxes that answer no port,
//                  and learns the name and model each gives itself.
//
// Every request goes through LocalOnly.check. That matters most here: a device
// answers discovery with an address of its own choosing, and the check refuses
// to follow one outside the home network.
...
class UPnPClient {
    ...
    fun discoverAll(listenMs: Int = 2500): Map<String, SsdpDevice> {
        ...
        try {
            ...
            DatagramSocket().use { sock ->
                ...
                repeat(2) { sock.send(DatagramPacket(search, search.size, group, SSDP_PORT)) }
                val end = System.currentTimeMillis() + listenMs
                val buf = ByteArray(2048)
                while (System.currentTimeMillis() < end) {
                    val reply = DatagramPacket(buf, buf.size)
                    try { sock.receive(reply) } catch (e: SocketTimeoutException) { continue }
                    val ip = reply.address?.hostAddress ?: continue
                    if (!LocalOnly.isAllowed(ip) || found.containsKey(ip)) continue
                    val text = String(reply.data, 0, reply.length, Charsets.ISO_8859_1)
                    found[ip] = SsdpDevice(ip, headerValue(text, "LOCATION"), headerValue(text, "SERVER"), headerValue(text, "USN"))
                }
            }
        } catch (e: Exception) {
            // no multicast on this network: discovery finds nothing and the sweep carries on
        }
        return found.mapValues { (_, d) -> withDescription(d) }
    }

    /** A device may describe only itself: its description must come from its own address. */
    private fun withDescription(d: SsdpDevice): SsdpDevice {
        val loc = d.location ?: return d
        val host = try { URI(loc).host } catch (e: Exception) { null }
        if (host != d.ip) return d
        val xml = httpGet(loc) ?: return d
        return d.copy(
            friendlyName = tag(xml, "friendlyName")?.let(::clean)?.take(60),
            manufacturer = tag(xml, "manufacturer")?.let(::clean)?.take(40),
            modelName = tag(xml, "modelName")?.let(::clean)?.take(40),
            deviceType = tag(xml, "deviceType")?.take(120),
        )
    }
    ...
    private fun discoverGateway(): String? {
        ...
        return try {
            ...
            DatagramSocket().use { sock ->
                ...
                val reply = DatagramPacket(buf, buf.size)
                sock.receive(reply)
                headerValue(String(reply.data, 0, reply.length, Charsets.ISO_8859_1), "LOCATION")
            }
        } catch (e: Exception) {
            null
        }
    ...
    private fun describe(location: String): Pair<String, String>? {
        ...
        for (st in SERVICE_TYPES) {
            ...
            val abs = try { URI(location).resolve(path).toString() } catch (e: Exception) { continue }
            return abs to st
        }
        return null
    }
...
<NewPortMappingIndex>$index</NewPortMappingIndex>
    ...
    private fun headerValue(text: String, name: String): String? {
        for (line in text.split("\r\n")) {
            if (line.uppercase().startsWith("${name.uppercase()}:")) return line.substringAfter(":").trim().ifEmpty { null }
        }
        return null
    }
